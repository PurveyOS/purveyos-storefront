...
export function calculateMixedRateTax(args: {
  ...
  for (const indexes of groups.values()) {
    ...
    if (args.pricesIncludeTax) {
      const netAmounts = allocateCents(
        grossTaxableCents - groupTaxCents,
        indexes.map((index) => snapshots[index].taxableAmountCents),
      )
      indexes.forEach((snapshotIndex, allocationIndex) => {
    ...
    breakdown.push({
      ...
      taxableAmountCents: args.pricesIncludeTax ? grossTaxableCents - groupTaxCents : grossTaxableCents,
